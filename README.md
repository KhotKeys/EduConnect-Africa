...
- `docker-compose.yml` — defines a `web` service that builds the image and exposes port 3000.
...
CI notes:
- The workflow runs `npm install`, `npm run lint`, `npm test`, and `docker build`.
- The pipeline is configured to fail if linting, tests, or Docker build fail.
