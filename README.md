...
This repository has been extended for Formative 2 with containerization and a CI pipeline.
...
2. The app will be available at http://localhost (port 80)

Files added for Formative 2:

- `Dockerfile` — container image using `python:3.11-slim`, runs app as non-root user.
- `docker-compose.yml` — defines the `web` service and maps port 8000.
- `.dockerignore` — keeps unnecessary files out of the Docker build context.
- `app.py` — minimal Flask app to serve the existing `frontend/` static files.
- `requirements.txt` / `requirements-dev.txt` — runtime and dev dependencies (Flask, pytest, flake8).
- `tests/test_app.py` — basic pytest verifying index page.
- `.github/workflows/ci.yml` — GitHub Actions pipeline that lints, tests, and builds the Docker image.

CI behavior:
- Triggers on pushes to branches other than `main` and on pull requests targeting `main`.
- Steps: checkout, set up Python 3.11, install deps, lint (flake8), run pytest, and build Docker image.

Note: For production-grade setups extend the pipeline to run security scans, build multi-arch images, and publish artifacts.
